...
public class Level3 extends GameThread {
    ...
    BadGuy badGuy = new BadGuy(mCanvasWidth,mCanvasHeight);
    //The speed (pixel/second) of the paddle in direction X and Y


    //Will store the image of the outhouse
    private Bitmap mOuthouse;

    //The X and Y position of the outhouse on the screen
    //The point is the top left corner, not middle, of the ball
    //Initially at -100 to avoid them being drawn in 0,0 of the screen
    private float mOuthouseX = -100;
    private float mOuthouseY = -100;

    //Will store the image of the wall and bad guys
    private Bitmap wall;
    private Bitmap badG;

    //The X and Y position of the wallss on the screen
    private float[] wallX = new float[50];
    private float[] wallY = new float[50];

    //This will store the min distance allowed between shrek and the onion
    //This is used to check collisions
    private float mMinDistanceBetweenBallAndPaddle = 0;

    //This is run before anything else, so we can prepare things here
    public Level3(GameView gameView) {
        //House keeping
        super(gameView);
        ...
        wall =  BitmapFactory.decodeResource
                (gameView.getContext().getResources(),
                        R.drawable.lava_level3);
        badG =  BitmapFactory.decodeResource
                (gameView.getContext().getResources(),
                        R.drawable.witch);
    }
    ...
    protected void doDraw(Canvas canvas) {
        ...
        canvas.drawBitmap(badG, badGuy.getX() - badGuy.getWidth() / 2, badGuy.getY() - badGuy.getHeight() / 2, null);
    ...
    protected void updateGame(float secondsElapsed) {
        ...
        if(updateBallCollision(badGuy.getX(), badGuy.getY())) {
            //go to next level
